...
class PriceHelper
{
    ...
    public static function OrderTotal($order,$trns=null)
    {
        ...
        foreach($cart as $key => $item){
            ...
            $cart_total = $total;
            if(Product::where('id',$key)->exists()){
                $item = Product::findOrFail($key);
                if(isset($item)){
                    if($item && $item->tax){
    ...
    public static function GetProductId($cart_id)
    {
    ...
    public static function LicenseQtyDecrese($cart)
    {
        foreach($cart as $item_id => $item){
            if($item['item_type'] == 'license'){
               $item = Product::findOrFail(PriceHelper::GetProductId($item_id));
    ...
    public static function stockDecrese()
    {
        $cart = Session::get('cart');
        foreach($cart as $key => $item){
            $main_item = Product::findOrFail($key);
