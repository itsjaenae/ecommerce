$(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".brand-select", function () {
          ...
          // catalog js start fabric
  $(document).on("click", ".fabric-select", function () {
    $('.fabric-select').prop('checked', false);
    let fabric = $(this).val();
    $(this).prop('checked', true);
    $("#search_form #fabric").val(fabric);
    removePage();
    $("#search_button").click();
});
  // catalog js start fit
$(document).on("click", ".fit-select", function () {
    $('.fit-select').prop('checked', false);
    let fit = $(this).val();
    $(this).prop('checked', true);
    $("#search_form #fit").val(fit);
    removePage();
    $("#search_button").click();
});
// catalog js start sleeve
$(document).on("click", ".sleeve-select", function () {
$('.sleeve-select').prop('checked', false);
let sleeve = $(this).val();
$(this).prop('checked', true);
$("#search_form #sleeve").val(sleeve);
removePage();
$("#search_button").click();
});
     // catalog js start pattern
$(document).on("click", ".pattern-select", function () {
    $('.pattern-select').prop('checked', false);
    let pattern = $(this).val();
    $(this).prop('checked', true);
    $("#search_form #pattern").val(pattern);
    removePage();
    $("#search_button").click();
    });
     // catalog js start occasion
$(document).on("click", ".occasion-select", function () {
    $('.occasion-select').prop('checked', false);
    let occasion = $(this).val();
    $(this).prop('checked', true);
    $("#search_form #occasion").val(occasion);
    removePage();
    $("#search_button").click();
    });
     // catalog js start neck
$(document).on("click", ".neck-select", function () {
    $('.neck-select').prop('checked', false);
    let neck = $(this).val();
    $(this).prop('checked', true);
    $("#search_form #neck").val(neck);
    removePage();
    $("#search_button").click();
    });
        ...
        function getData(status = 0, check = 0, item_key = 0, qty = 0, add_type = 0) {
            ...
            if (status == 1) {
                let addToCartUrl = `${mainurl}/product/add/cart?product_id=${itemId}&options_ids=${options_ids}&attribute_ids=${attribute_ids}&quantity=${quantity}&type=${type}&item_key=${item_key}&add_type=${add_type}`;
                $.ajax({
                    type: "GET",
                    url: addToCartUrl,
                    success: function (data) {
                        ...
                        if (add_type == 1) {
                            location.href = mainurl + '/user/cart';
                        } else {
                            successNotification(data.message);
                        }
