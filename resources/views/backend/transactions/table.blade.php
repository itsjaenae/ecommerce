...
<tr id="transaction-bulk-delete">
    ...
    <td >
        ...
        {{$data->user_email}}
        @else
        <a href="{{route('admin.user.show',$data->user->id)}}">{{$data->user_email}}</a>
        @endif
        
    </td>
    <td>
        <a href="{{route('admin.order.invoice',$data->order_id)}}">{{ $data->txn_id}}</a>
    </td>
    <td>
    ...
    <td>
        <div class="action-list">
            <a class="btn btn-danger btn-sm " data-toggle="modal"
                ...
                data-href="{{ route('admin.transaction.delete',$data->id) }}">
