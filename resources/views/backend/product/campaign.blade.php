        ...
        <div class="card-header">
            ...
            <div class="row">
                <form action="{{route('admin.campaign.store')}}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="form-group ">
                            <select id="basic" name="product_id" class="form-control" >
                ...
                <tbody>
                    @if ($items->count() > 0)
                      @foreach ($items as $data)
                          <tr>
                              <td>
                                <img src="{{asset('images/product_images/'.$data->item->photo)}}" alt="">
                            </td>
                              <td>{{$data->item->name_en}}</td>
                              ...
                              <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
                                      <a class="dropdown-item" href="{{ route('admin.campaign.status',[$data->id,1,'status']) }}">{{ __('Publish') }}</a>
                                      <a class="dropdown-item" href="{{ route('admin.campaign.status',[$data->id,0,'status']) }}">{{ __('Unpublish') }}</a>
                                    </div>
                                  </div>
                            </td>
                              <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
                                      <a class="dropdown-item" href="{{ route('admin.campaign.status',[$data->id,1,'is_feature']) }}">{{ __('Active') }}</a>
                                      <a class="dropdown-item" href="{{ route('admin.campaign.status',[$data->id,0,'is_feature']) }}">{{ __('Deactive') }}</a>
                                    </div>
                                  </div>
                            </td>
                              <td>
                                <a class="btn btn-danger btn-sm " data-toggle="modal"
                                    ...
                                    data-href="{{ route('admin.campaign.destroy',$data->id) }}">
