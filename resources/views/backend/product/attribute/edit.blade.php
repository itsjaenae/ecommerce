			...
			<div class="card o-hidden border-0 shadow-lg">
				<div class="card-body ">
					...
					<div class="row justify-content-center">
						<div class="col-lg-12">
								<form class="admin-form" action="{{ route('admin.attribute.update',[$product->id,$attribute->id]) }}"
									...
									<div class="row">
										...
										<div class="form-group">
											...
											<select name="name" class="form-control" id="name" >
												...
												{{-- @foreach(DB::table('colors')->whereStatus(1)->get() as $attr) --}}
												<option value="{{ $attribute->id }}" {{ $attribute->id == $attribute->name ? 'selected' : '' }}>
													{{ $attribute->name }}
												</option>
										...
										{{-- <div class="form-group">
											<select type="text" class="form-control"
												...
												@foreach(DB::table('sizes')->whereStatus(1)->get() as $attr)
												<option value="{{ $attr->id }}" {{ $attr->id == $attribute->size ? 'selected' : ''}}>{{ $attr->size }}</option>
												@endforeach
											</select>
											</div>
										</div> --}}
