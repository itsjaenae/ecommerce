    ...
    <div class="table-responsive shopping-cart">
        <table class="table table-bordered">
          ...
          <tbody id="cart_view_load" data-target="{{route('cart.get.load')}}">
            ...
            <tr>
                <td>
                  <div class="product-item"><a class="product-thumb" href="{{route('product.details',$item['slug_en'])}}"><img src="{{asset('images/product_images/'.$item['photo'])}}" alt="Product"></a>
                    <div class="product-info">
                      <h4 class="product-title"><a href="{{route('product.details',$item['slug_en'])}}">
                        {{ strlen(strip_tags($item['name_en'])) > 45 ? substr(strip_tags($item['name_en']), 0, 45) . '...' : strip_tags($item['name_en']) }}
                ...
                <td class="text-center">
                 @if ($item['item_type'] != 'digital')
                    ...
                    <span class="decreaseQtycart cartsubclick" data-id="{{$key}}"
                     data-target="{{PriceHelper::GetProductId($key)}}"><i class="fas fa-minus"></i></span>
                    ...
                    data-target="{{PriceHelper::GetProductId($key)}}"><i class="fas fa-plus"></i></span>
...
@php
   $coupon = App\Models\PromoCode::where('status',1)->first();
@endphp

{{-- @if(isset($coupon->code_name) )
<span class="text-muted">{{__('use Coupon Discount Code')}}
    <h4>{{$coupon->code_name}}</h4>
    </span>
    @else    
    @endif  --}}
      ...
      <div class="card-body">
        <div class="shopping-cart-footer">
            ...
            <div class="text-right text-lg column {{Session::has('coupon') ? '' : 'd-none'}}">
              <span class="text-muted">{{__('Discount')}} ({{Session::has('coupon')
               ? Session::get('coupon')['code']['code_name'] : ''}}) : </span><span class="text-gray-dark">{{PriceHelper::setCurrencyPrice(Session::has('coupon') ? Session::get('coupon')['discount'] : 0)}}</span></div>
    ...
    <div class="shopping-cart-footer">
        <div class="column"><a class="btn btn-primary " href="{{route('all.catalog')}}">
          <span><i class="icon-arrow-left"></i> {{__('Back to Shopping')}}</span></a></div>
        	@guest
     <div class="column"><a class="btn btn-primary" href="{{ route('login') }}">
        <span>{{__('Checkout')}}</span></a></div>
			@else 
        <div class="column"><a class="btn btn-primary" href="{{route('front.checkout.billing')}}">
        <span>{{__('Checkout')}}</span></a></div>
        @endif
    </div>
  </div>
